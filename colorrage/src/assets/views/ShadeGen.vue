<template>
    <div class="w-full flex flex-col justify-center md:scale-[.9]">
        <h1 class="font-extrabold flex text-2xl tracking-tighter text-white justify-center mb-10">COLOR SHADES</h1>
        <div class="grid grid-rows-2 md:grid-rows-0 md:grid-cols-4 justify-between gap-4 w-full">
        ...
        <div class="flex flex-col justify-center w-full mt-10 md:-mt-5 md:flex-row gap-2 relative">
            
            <OutputColorSquare  v-for="(color, index) in shades.slice(0, 4)" :key="`left-${index}`" :color="color" class="self-center"/>
            
            <InputColorSquare v-model="inputColor" class="self-center" />

            <OutputColorSquare v-for="(color, index) in shades.slice(5)" :key="`right-${index}`" :color="color" class="self-center"/>
        </div>
